"""
This code is adapted and improved from the original code by Yuhuang Hu and Tobi Delbruck.

Original Authors: Yuhuang Hu, Tobi Delbruck
Original Emails : yuhuang.hu@ini.uzh.ch, tobi@ini.uzh.ch
"""
...
def low_pass_filter(
    ...
    # Returns
        first_stage: the result of the first stage of the filter
    """
    if cutoff_hz <= 0:
        # unchanged
    ...
    if inten01 is not None:
        ...
        if max_eps > 0.3:
            ...
            if low_pass_filter.iir_warning_count < IIR_MAX_WARNINGS:
                ...
                if low_pass_filter.iir_warning_count == IIR_MAX_WARNINGS:
                    logger.warning(f'Suppressing further warnings about inaccurate IIR lowpass filtering; check timestamp resolution and DVS photoreceptor cutoff frequency')
    ...
    if not torch.cuda.is_available():
        raise SystemError("CUDA is not available. Check your PyTorch installation and GPU.")

    mu_eps = 0.5  
    sigma_eps = 0.1  
    threshold = 0.5 
    gaussian_matrix = torch.normal(mean=mu_eps, std=sigma_eps, size=eps.size(), device='cuda')
    probability_mask = gaussian_matrix > threshold
    eps = torch.where(probability_mask, torch.ones_like(eps), eps)

    # save_lpf_image(log_new_frame, low_pass_filter.first_stage_count,"lgnew")
    # low_pass_filter.first_stage_count += 1

    # Apply the first stage of the filter
    first_stage = (1 - eps) * lp_log_frame + eps * log_new_frame

    # save_lpf_image(first_stage, low_pass_filter.first_stage_count,"fs")
    # low_pass_filter.first_stage_count += 1

    return first_stage

low_pass_filter.iir_warning_count=0
# low_pass_filter.first_stage_count = 0
# def save_lpf_image(tensor, count,name="a"):
#     """Save the tensor as an image with a given count as filename suffix."""
#     print(torch.mean(tensor))
#     tensor = tensor/255.0
#     tensor = tensor.cpu() 
#     transform = transforms.ToPILImage()
#     img = transform(tensor)
#     img_path = os.path.join(SAVE_DIR, name+f'_{count}.png')
#     img.save(img_path)
