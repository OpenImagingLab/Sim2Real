...
class EventRenderer(object):
    ...
    def e2f(self, event_arr:np.array,
            height:int, width: int,
            return_frames=False)-> np.ndarray:
        # @njit()
        def events2frame(event_frame, events):
            event_len = np.shape(events)[0]
            for ei in range(event_len):
                event_frame[int(events[ei, 2]), int(events[ei, 1])] = events[ei, 3]
            return event_frame

        event_frame = np.zeros((height, width), dtype=np.float32)
        event_frame = events2frame(event_frame, event_arr)
        
        img = np.uint8((event_frame*0.5+0.5)*255) # event中只包含+1 -1值，请将他们转换成红蓝色图片显示，而不是灰度
        print(np.max(event_frame),np.min(event_frame),np.min(img),np.max(img))
        if self.video_output_file:
            self.video_output_file.write(
                cv2.cvtColor((img).astype(np.uint8),
                             cv2.COLOR_GRAY2BGR))
            t = None
        ...
        if self.preview:
            ...
            if k == 27 or k == ord('x'):
                v2e_quit()
        return event_frame, img

    def e2f_red_blue(self, event_arr:np.array,
            height:int, width: int,
            return_frames=False)-> np.ndarray:
        # @njit()
        def events2frame(event_frame, events):
        ...
        # Initialize an RGB image with a white background
        rgb_img = np.full((height, width, 3), 255, dtype=np.uint8)

   
        # Initialize dictionaries to count the occurrences of each coordinate
        red_counts = {}
        blue_counts = {}
        
        # Processing event data for red and blue color representation
        red_indices = event_arr[:, 3] == 1   # Indices where event values are +1
        blue_indices = event_arr[:, 3] == -1  # Indices where event values are -1

        # Count occurrences of red and blue coordinates
        for x, y in zip(event_arr[red_indices, 2].astype(int), event_arr[red_indices, 1].astype(int)):
            red_counts[(x, y)] = red_counts.get((x, y), 0) + 1

        for x, y in zip(event_arr[blue_indices, 2].astype(int), event_arr[blue_indices, 1].astype(int)):
            blue_counts[(x, y)] = blue_counts.get((x, y), 0) + 1

        # Set pixels based on the highest count for each coordinate
        for coord, count in red_counts.items():
            if count > blue_counts.get(coord, 0):  # Red has a higher count
                rgb_img[coord] = [0, 0, 255]  # Set red pixels

        for coord, count in blue_counts.items():
            if count > red_counts.get(coord, 0):  # Blue has a higher count or only blue
                rgb_img[coord] = [255, 0, 0]  # Set blue pixels
        ...
        # # Set the red channel to 255 for +1 events
        # # rgb_img[event_arr[red_indices, 2].astype(int), event_arr[red_indices, 1].astype(int), 0] = 255

        # # Set the blue pixels (and reset red and green channels to 0 for these pixels)
        # red_x, red_y = event_arr[red_indices, 2].astype(int), event_arr[red_indices, 1].astype(int)
        # rgb_img[red_x, red_y, 0] = 0  # Blue channel
        # rgb_img[red_x, red_y, 1] = 0  # Green channel
        # rgb_img[red_x, red_y, 2] = 255  # Red channel


        # # Set the blue pixels (and reset red and green channels to 0 for these pixels)
        # blue_x, blue_y = event_arr[blue_indices, 2].astype(int), event_arr[blue_indices, 1].astype(int)
        # rgb_img[blue_x, blue_y, 0] = 255  # Blue channel
        # rgb_img[blue_x, blue_y, 1] = 0  # Green channel
        # rgb_img[blue_x, blue_y, 2] = 0  # Red channel
