    ...
    if synthetic_input is not None:
        try:
            ...
            synthetic_input_instance:base_synthetic_input = synthetic_input_class(
                ...
                preview=not args.no_preview, arg_list=other_args, avi_path=vid_path,parent_args=args, key=args.file_key
...
#         v2e_quit(1)
   ...
   # input file checking
    ...
    if synthetic_input_next_frame_method is not None:
        ...
        num_frames+=1
        i = 0
        file_name = synthetic_input_instance.file_name
        events_list = []
        names_list = []
        with tqdm(total=synthetic_input_instance.total_frames()-1,
            ...
            with torch.no_grad():
                while fr is not None:
                    ...
                    if newEvents is not None and newEvents.shape[0] > 0:
                        event_frame, img = eventRenderer.e2f_red_blue(newEvents, output_height, output_width, True)
                        cv2.imwrite(os.path.join(synthetic_input_instance.event_vis_path, f"{file_name}.jpg"),
                                    img)
                        # # save event frame use np.savez
                        # np.savez_compressed(os.path.join(synthetic_input_instance.event_path, file_name),  
                        #                     data=np.int8(event_frame))
                        
                        temp_dir = tempfile.mkdtemp()
                        event_temp_path = os.path.join(temp_dir, f"{file_name}.npy")
                        # print(event_temp_path)
                        
                        np.save(event_temp_path, np.int8(event_frame))
                        shutil.move(event_temp_path, os.path.join(synthetic_input_instance.event_path, f"{file_name}.npy"))

                        names_list.append(file_name)
                        events_list.append(event_frame)
                        # if len(names_list) == len(events_list) and len(events_list) == synthetic_input_instance.mul:
                        #     curacc = np.sum(np.stack(events_list).astype(np.float32), 0).clip(-1, 1)
                        #     curacc = (curacc*0.5+0.5)*255
                        #     cv2.imwrite(os.path.join(synthetic_input_instance.event_vis_acc_path, f"{names_list[0]}.jpg"),
                        #                 np.uint8(curacc))
                        #     events_list = []
                        #     names_list = []
                    # if newEvents is not None and newEvents.shape[0] > 0 \
