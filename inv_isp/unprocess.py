...
def apply_ccm(image, ccm):
  ...
  # print("ssssssssssssssss",image.shape,ccm.shape)
...
def unprocess_event(image, rgb2cam ,rgb_gain, red_gain, blue_gain, low_light_factor):
    ...
    image = safe_invert_gains(image, rgb_gain * low_light_factor, red_gain, blue_gain)
...
def random_noise_levels(min_shot=0.0001,max_shot=0.012,min_read=0.0,max_read=0.26):
  """Generates random noise levels from a log-log linear distribution."""
  log_min_shot_noise = np.log(min_shot)
  log_max_shot_noise = np.log(max_shot)
  ...
  n    = tdist.Normal(loc=torch.tensor([min_read]), scale=torch.tensor([max_read])) 
...
def random_noise_levels_kpn(min_shot=-4.0,max_shot=-2.0,min_read=-3.0,max_read=-1.5):
  sigma_read = torch.from_numpy(
    np.power(10, np.random.uniform(min_read, max_read, (1,))))
  # sigma_read = sigma_read**2
  sigma_shot = torch.from_numpy(
    np.power(10, np.random.uniform(min_shot, max_shot, (1,))))
...
def add_noise(image, shot_noise=0.01, read_noise=0.0005):
  ...
  noise    = n.sample()
  # print(shot_noise, read_noise, noise)
  out      = image + noise
  out      = out.permute(2, 0, 1) # Re-Permute the tensor back to CxHxW format
  return out #, noise
